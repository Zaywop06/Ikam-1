...
const App = () => { 
  ...
  useEffect(() => {
    const pyme = pymes.filter(p =>
      p.coloniaPyme && typeof p.coloniaPyme === 'string' && p.coloniaPyme.includes(colonia)
    );
    setPymesCol(pyme)
  }, [pymes]);
  ...
  useEffect(() => {
    if (categoriasElegir) {
      const pyme = pymesCol.filter(p =>
        // p.nombre_pyme.toLowerCase().includes(busquedaPyme.toLowerCase())
        p.subcatPyme && typeof p.subcatPyme === 'string' && p.subcatPyme.includes(categoriasElegir)
      );
      setPymesQ(pyme);
    }
  }, [categoriasElegir])
